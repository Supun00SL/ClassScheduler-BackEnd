 ...
 */
@WebServlet(name = "sheduleclass", urlPatterns = {"/sheduleclass"})
public class sheduleclass extends HttpServlet {
        ...
        try {
            ...
            for (Subjectvisebatch subjectvisebatch : subjectvisebatchs) {
                ...
                if (request.getParameter(subjectvisebatch.getIdsubjectvisebatch().toString()).equals("on")) {
                    pojos.Normalclassshedule normalclassshedule = new Normalclassshedule();

                    normalclassshedule.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(request.getParameter("date")));
                    normalclassshedule.setEndtime(new SimpleDateFormat("HH:mm").parse(request.getParameter("endtime")));
                    normalclassshedule.setStarttime(new SimpleDateFormat("HH:mm").parse(request.getParameter("starttime")));
                    ...
                    normalclassshedule.setLecturehall(lecturehall);
                    normalclassshedule.setLocation(request.getParameter("location"));
                    normalclassshedule.setState("active");
                    normalclassshedule.setSubjectvisebatch(subjectvisebatch);

                    session.save(normalclassshedule);
                    ...
                    for (StudentHasSubjectvisebatch studentHasSubjectvisebatch : hasSubjectvisebatchs) {
                        ...
                        specialnotifications.setDescription("Your " + studentHasSubjectvisebatch.getSubjectvisebatch().getSubject().getSubject() + ""
                                + " Class will be held on " + dateFormat.format(dateFormat.parse(request.getParameter("date"))) + " at "
                        ...
                        specialnotifications.setHeader("New Class Sheduled !");
                        ...
                        pushnoti.AndroidPush.sendPushNotification("New Class Sheduled !", "Your " + studentHasSubjectvisebatch.getSubjectvisebatch().getSubject().getSubject() + ""
                                + " Class will be held on " + dateFormat.format(dateFormat.parse(request.getParameter("date"))) + " at "
                                + "" + timeFormat.format(timeFormat.parse(request.getParameter("starttime"))) + " to " + timeFormat.format(timeFormat.parse(request.getParameter("endtime")))
            ...
            t.commit();
            session.flush();
            response.sendRedirect("ClassSchedule.jsp?id=1");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            out.close();
